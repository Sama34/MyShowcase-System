...
if($mybb->input['action'] == 'new')
{
	...
	if($mybb->request_method == 'post')
	{
		...
		if(!empty($mybb->input['newname']))
		{
			...
			{
				flash_message($lang->sprintf($lang->myshowcase_fields_lang_not_writable,$lang->path.'/'.$lang->language.'/'), 'error');
...
if($mybb->input['action'] == 'editset')
{
	...
	if(isset($mybb->input['setid']) && is_numeric($mybb->input['setid']))
	{
		...
		$can_edit_lang = true;
		if(!is_writable($langpath) || (!is_writable($langpath.'/myshowcase_fs'.$mybb->input['setid'].'.lang.php') && file_exists($langpath.'/myshowcase_fs'.$mybb->input['setid'].'.lang.php')))
		{
			$can_edit_lang = false;
		}
		...
		$page->add_breadcrumb_item($lang->sprintf($lang->myshowcase_admin_edit_fieldset, $result['setname']), "index.php?module=myshowcase-fields");
		$page->output_header($lang->myshowcase_admin_fields);
		...
		if($mybb->request_method == "post")
		{
			...
			while($result = $db->fetch_array($query))
			{
				...
				$update_query = $db->update_query("myshowcase_fields", $update_array, "setid=".$mybb->input['setid']." AND fid=".$result['fid']);
				...
				if($can_edit && ($mybb->input['html_type'][$result['fid']] == 'db' || $mybb->input['html_type'][$result['fid']] == 'radio' ))
				{
					...
					$update_query = $db->update_query("myshowcase_field_data", $update_array, "setid=".$mybb->input['setid']." AND fid=".$result['fid']);
				}
			}

			//apply new field
			$do_new_field = 0;
			if($mybb->input['newname'] != "" && $mybb->input['newmax_length'] != "" && is_numeric($mybb->input['newmax_length']) && $mybb->input['newfield_order'] != "" && is_numeric($mybb->input['newfield_order'])  && $mybb->input['newlist_table_order'] != "" && is_numeric($mybb->input['newlist_table_order']))
			{
			...
			if($can_edit_lang)
			{
				...
				{
					$items_add['myshowcase_field_'.$value] = ($mybb->input['label'][$key] == "" ? $db->escape_string($mybb->input['name'][$key]) : $db->escape_string($mybb->input['label'][$key]));
				}

				//write new field from input
				if($do_new_field)
				{
					$items_add['myshowcase_field_'.$db->escape_string($mybb->input['newname'])] = ($mybb->input['newlabel'] == "" ? $db->escape_string($mybb->input['newname']) : $db->escape_string($mybb->input['newlabel']));
				}
		...
		$form = new Form("index.php?module=myshowcase-fields&amp;action=editset&amp;setid=".$mybb->input['setid'], "post", "editset");
		$form_container = new FormContainer($lang->myshowcase_field_list);

		$form_container->output_row_header($lang->myshowcase_field_fid, array("width" => "5%", "class" => "align_center"));
		$form_container->output_row_header($lang->myshowcase_field_name, array("width" => "10%", "class" => "align_center"));
		$form_container->output_row_header($lang->myshowcase_field_label, array("width" => "10%", "class" => "align_center"));
		$form_container->output_row_header($lang->myshowcase_field_html_type, array("width" => "5%", "class" => "align_center"));
		$form_container->output_row_header($lang->myshowcase_field_field_type, array("width" => "5%", "class" => "align_center"));
		$form_container->output_row_header($lang->myshowcase_field_min_length, array("width" => "5%", "class" => "align_center"));
		$form_container->output_row_header($lang->myshowcase_field_max_length, array("width" => "5%", "class" => "align_center"));
		$form_container->output_row_header($lang->myshowcase_field_field_order, array("width" => "5%", "class" => "align_center"));
		$form_container->output_row_header($lang->myshowcase_field_list_table_order, array("width" => "4%", "class" => "align_center"));
		$form_container->output_row_header($lang->myshowcase_field_format, array("width" => "10%", "class" => "align_center"));
		...
		$query = $db->simple_select("myshowcase_fields", "*", "setid=".$mybb->input['setid'], array("order_by" => "fid"));
		$num_fields = $db->num_rows($query);
		if($num_fields == 0)
		{
			$form_container->output_cell($lang->myshowcase_fields_no_fields, array("class" => "align_center", 'colspan' => 15));
		...
		else
		{
			...
			while($result = $db->fetch_array($query))
			{
				...
				$popup->add_item($lang->myshowcase_field_delete, "index.php?module=myshowcase-fields&amp;action=delfield&amp;fid={$result['fid']}&amp;setid=".$mybb->input['setid']);

				//add option to edit list items if db type
				if($result['html_type'] == 'db' || $result['html_type'] == 'radio')
				{
					$popup->add_item($lang->myshowcase_field_edit_options, "index.php?module=myshowcase-fields&amp;action=editopt&amp;fid={$result['fid']}&amp;setid=".$mybb->input['setid']);
				}
				...
				if($can_edit)
				{
					$form_container->output_cell($form->generate_text_box('name['.$result['fid'].']', $result['name'], array('id' => 'label['.$result['fid'].']', 'style' => 'width: 100px')),array('class' => 'align_left'));
				}
				else
				{
					$form_container->output_cell($result['name'].$form->generate_hidden_field('name['.$result['fid'].']',$result['name']),array('class' => 'align_left'));
				}
				if($can_edit_lang)
				{
					$form_container->output_cell($form->generate_text_box('label['.$result['fid'].']', $l['myshowcase_field_'.$result['name']], array('id' => 'label['.$result['fid'].']', 'style' => 'width: 100px')),array('class' => 'align_left'));
				}
				else
				{
					$form_container->output_cell($l['myshowcase_field_'.$result['name']],array('class' => 'align_left'));
				}
				$form_container->output_cell($result['html_type'].$form->generate_hidden_field('html_type['.$result['fid'].']',$result['html_type']), array("class" => "align_center"));
				$form_container->output_cell($result['field_type'].$form->generate_hidden_field('field_type['.$result['fid'].']',$result['field_type']), array("class" => "align_center"));
				$form_container->output_cell($result['min_length'].$form->generate_hidden_field('min_length['.$result['fid'].']',$result['min_length']),array("class" => "align_center"));
				$form_container->output_cell($result['max_length'].$form->generate_hidden_field('max_length['.$result['fid'].']',$result['max_length']),array("class" => "align_center"));
				$form_container->output_cell($form->generate_text_box('field_order['.$result['fid'].']', $result['field_order'], array('id' => 'field_order['.$result['fid'].']', 'style' => 'width: 35px')),array("class" => "align_center"));
				$form_container->output_cell($form->generate_text_box('list_table_order['.$result['fid'].']', $result['list_table_order'], array('id' => 'list_table_order['.$result['fid'].']', 'style' => 'width: 35px')),array("class" => "align_center"));
				$form_container->output_cell($form->generate_select_box('format['.$result['fid'].']', $field_format_options, $result['format'], array('id' => 'format['.$result['fid'].']')),array('class' => 'align_center'));
				$form_container->output_cell($form->generate_check_box("enabled[".$result['fid']."]", 'true', null, array('checked' => $result['enabled']), ""),array("class" => "align_center"));
				$form_container->output_cell($form->generate_check_box("require[".$result['fid']."]", 'true', null, array('checked' => $result['require']), ""),array("class" => "align_center"));
				$form_container->output_cell($form->generate_check_box("parse[".$result['fid']."]", 'true', null, array('checked' => $result['parse']), ""),array("class" => "align_center"));
				$form_container->output_cell($form->generate_check_box("searchable[".$result['fid']."]", 'true', null, array('checked' => $result['searchable']), ""),array("class" => "align_center"));
...
/*			$form_container->output_row_header($lang->myshowcase_field_enabled, array("width" => "4%", "class" => "align_center"));
			...
			$form_container->output_cell($form->generate_text_box('newname', "", array('id' => 'newname', 'style' => 'width: 100px')),array('class' => 'align_center'));
			$form_container->output_cell($form->generate_text_box('newlabel', "", array('id' => 'newlabel', 'style' => 'width: 100px')),array('class' => 'align_center'));
			$form_container->output_cell($form->generate_select_box('newhtml_type', $html_type_options, "", array('id' => 'newhtml_type')),array('class' => 'align_center'));
			$form_container->output_cell($form->generate_select_box('newfield_type', $field_type_options, "", array('id' => 'newfield_type')),array('class' => 'align_center'));
			$form_container->output_cell($form->generate_text_box('newmin_length', "", array('id' => 'newmin_length', 'style' => 'width: 50px')),array('class' => 'align_center'));
			$form_container->output_cell($form->generate_text_box('newmax_length', "", array('id' => 'newmax_length', 'style' => 'width: 50px')),array('class' => 'align_center'));
			$form_container->output_cell($form->generate_text_box('newfield_order', $max_order+1, array('id' => 'newfield_order', 'style' => 'width: 35px')),array('class' => 'align_center'));
			$form_container->output_cell($form->generate_text_box('newlist_table_order', -1, array('id' => 'newlist_table_order', 'style' => 'width: 35px')),array('class' => 'align_center'));
			$form_container->output_cell($form->generate_select_box('newformat', $field_format_options, "", array('id' => 'newformat')),array('class' => 'align_center'));
...
if($mybb->input['action'] == 'delset')
{
	...
	if(isset($mybb->input['setid']) && is_numeric($mybb->input['setid']))
	{
		...
		{
			$page->add_breadcrumb_item($lang->sprintf($lang->myshowcase_admin_edit_fieldset, $setname), "index.php?module=myshowcase-fields");
			$page->output_header($lang->myshowcase_admin_fields);
			...
			else
			{
				...
				$form = new Form("index.php?module=myshowcase-fields&amp;action=do_delset&amp;setid=".$mybb->input['setid'], "post", "do_delset");
...
if($mybb->input['action'] == 'editopt')
{
	...
	if(isset($mybb->input['fid']) && is_numeric($mybb->input['fid']))
	{
		...
		$query = $db->simple_select("myshowcase_fields", "*", "setid=".$mybb->input['setid']. " AND fid=".$mybb->input['fid']. " AND html_type In ('db', 'radio')");
		if($db->num_rows($query) == 0)
		{
		...
		$page->add_breadcrumb_item($lang->sprintf($lang->myshowcase_admin_edit_fieldopt, $fieldname, $setname), "index.php?module=myshowcase-fields");
		$page->output_header($lang->myshowcase_admin_fields);
		...
		{
			//apply changes to existing fields first
			$query = $db->simple_select("myshowcase_field_data", "*", "setid=".$mybb->input['setid']. " AND fid=".$mybb->input['fid']);
			while($result = $db->fetch_array($query))
			{
				$update_array = array(
				...
				$update_query = $db->update_query("myshowcase_field_data", $update_array, "setid=".$mybb->input['setid']." AND fid=".$mybb->input['fid']." AND valueid=".$result['valueid']);
			}

			//apply new field
			if($mybb->input['newtext'] != "" && $mybb->input['newvalueid'] != "" && is_numeric($mybb->input['newvalueid']) && $mybb->input['newdisporder'] != "" && is_numeric($mybb->input['newdisporder']))
			{
				$insert_array = array(
					"setid" => $mybb->input['setid'],
					"fid" => $mybb->input['fid'],
			...
			admin_redirect("index.php?module=myshowcase-fields&amp;action=editopt&amp;fid=".$mybb->input['fid']."&amp;setid=".$mybb->input['setid']);
		}

		$form = new Form("index.php?module=myshowcase-fields&amp;action=editopt&amp;fid=".$mybb->input['fid']."&amp;setid=".$mybb->input['setid'], "post", "editopt");
		$form_container = new FormContainer($lang->myshowcase_field_list);

		$form_container->output_row_header($lang->myshowcase_field_fdid, array("width" => "5%", "class" => "align_center"));
		$form_container->output_row_header($lang->myshowcase_field_option_text, array("width" => "10%", "class" => "align_center"));
		$form_container->output_row_header($lang->myshowcase_field_disporder, array("width" => "5%", "class" => "align_center"));
		$form_container->output_row_header($lang->controls, array("width" => "10%", "class" => "align_center"));

		$query = $db->simple_select("myshowcase_field_data", "*", "setid=".$mybb->input['setid']." AND fid=".$mybb->input['fid'], array("order_by" => "valueid"));
		$num_fields = $db->num_rows($query);
		if($num_fields == 0)
		{
			$form_container->output_cell($lang->myshowcase_field_no_options, array("class" => "align_center", 'colspan' => 5));
			$form_container->construct_row();
		}
		else
		{
			...
			while($result = $db->fetch_array($query))
			{
				...
				if($can_edit)
				{
					$popup->add_item($lang->myshowcase_field_delete, "index.php?module=myshowcase-fields&amp;action=delopt&amp;setid={$mybb->input['setid']}&amp;fid={$mybb->input['fid']}&amp;valueid={$result['valueid']}");
				}

				$max_order = max($max_order, $result['disporder']);
				$max_valueid = max($max_valueid, $result['valueid']);
				$form_container->output_cell($result['valueid'].$form->generate_hidden_field('fieldname['.$result['valueid'].']',$result['name']),array("class" => "align_center"));

				$form_container->output_cell($form->generate_text_box('value['.$result['valueid'].']', $result['value'], array('id' => 'value['.$result['valueid'].']', 'style' => 'width: 105px')),array("class" => "align_center"));

				$form_container->output_cell($form->generate_text_box('disporder['.$result['valueid'].']', $result['disporder'], array('id' => 'disporder['.$result['valueid'].']', 'style' => 'width: 65px')),array("class" => "align_center"));
				if($can_edit)
				{
					$form_container->output_cell($popup->fetch(),array("class" => "align_center"));
				}
		...
		$form_container->output_row_header($lang->myshowcase_field_option_text." *", array("width" => "65", "class" => "align_center"));
		$form_container->output_row_header($lang->myshowcase_field_disporder, array("width" => "65", "class" => "align_center"));
		...
		$form_container->output_cell($form->generate_text_box('newdisporder', $max_order+1, array('id' => 'newdisporder', 'style' => 'width: 150px')),array('class' => 'align_center'));
...
if($mybb->input['action'] == 'delopt')
{
	...
	if(isset($mybb->input['valueid']) && is_numeric($mybb->input['valueid']))
	{
		...
		$query = $db->simple_select("myshowcase_field_data", "*", "setid=".$mybb->input['setid']." AND fid=".$mybb->input['fid']." AND valueid=".$mybb->input['valueid']);
		$result = $db->fetch_array($query);
		$fieldname = $result['name'];
		if($db->num_rows($query) == 0)
		{
		...
		else
		{
			...
			{
				$query = $db->delete_query("myshowcase_field_data", "setid=".$mybb->input['setid']." AND fid=".$mybb->input['fid']." AND valueid=".$mybb->input['valueid']);
				...
				// Log admin action
				$log = array('setid' => $mybb->input['setid'],'fid' => $mybb->input['fid'],'valueid' => $mybb->input['valueid']);
				log_admin_action($log);

				flash_message($lang->myshowcase_field_delete_opt_success, 'success');
				admin_redirect("index.php?module=myshowcase-fields&amp;action=editopt&amp;setid=".$mybb->input['setid']."&amp;fid=".$mybb->input['fid']);
			}
		}
	}
}
...
if($mybb->input['action'] == 'delfield')
{
	...
	if(isset($mybb->input['fid']) && is_numeric($mybb->input['fid']))
	{
		...
		$query = $db->simple_select("myshowcase_fields", "name", "setid=".$mybb->input['setid']." AND fid=".$mybb->input['fid']);
		$result = $db->fetch_array($query);
		$fieldname = $result['name'];
		if($fieldname == "")
		{
		...
		else
		{
			...
			else
			{
				...
				$form = new Form("index.php?module=myshowcase-fields&amp;action=do_delfield&amp;fid=".$mybb->input['fid']."&amp;setid=".$mybb->input['setid'], "post", "do_delete");
...
if($mybb->input['action'] == 'do_delfield')
{
	...
	if(isset($mybb->input['fid']) && is_numeric($mybb->input['fid']) && $mybb->request_method == 'post')
	{
		...
		//delete actual field
		$query = $db->delete_query("myshowcase_fields", "setid=".$mybb->input['setid']." AND fid=".$mybb->input['fid']);
		if($db->affected_rows($query) != 1)
		{
		...
		$query = $db->delete_query("myshowcase_field_data", "setid=".$mybb->input['setid']." AND fid=".$mybb->input['fid']);

		//edit language file if can be edited
		$retval = modify_lang('myshowcase_fs'.$mybb->input['setid'], array(), array('myshowcase_field_'.$fieldname => ""), "english", false);
...
if($mybb->input['action'] == '')
{
	...
	$form_container->output_row_header($lang->myshowcase_fields_count, array("width" => "25%", "class" => "align_center"));
	$form_container->output_row_header($lang->myshowcase_fields_assigned_to, array("width" => "10%", "class" => "align_center"));
	$form_container->output_row_header($lang->myshowcase_fields_used_by, array("width" => "10%", "class" => "align_center"));
	$form_container->output_row_header($lang->myshowcase_fields_lang_exists, array("width" => "10%", "class" => "align_center"));
	$form_container->output_row_header($lang->controls, array("width" => "10%", "class" => "align_center"));
	...
	else
	{
		while($result = $db->fetch_array($query))
		{
			$query_used_in = $db->query("SELECT count(*) AS total FROM ".TABLE_PREFIX."myshowcase_config WHERE fieldsetid=".$result['setid']);
			$num_used = $db->fetch_array($query_used_in);

			$query_fields = $db->query("SELECT count(*) AS total FROM ".TABLE_PREFIX."myshowcase_fields WHERE setid=".$result['setid']);
			...
			$popup->add_item($lang->edit, "index.php?module=myshowcase-fields&amp;action=editset&amp;setid={$result['setid']}");
			$popup->add_item($lang->delete, "index.php?module=myshowcase-fields&amp;action=delset&amp;setid={$result['setid']}");
			...
			$form_container->output_cell($form->generate_text_box("setname[".$result['setid']."]",$result['setname']));
			...
			$form_container->output_cell('<img src="'.$status_image.'" title="'.$status_alt.'">',array("class" => "align_center"));
			$form_container->output_cell($popup->fetch(),array("class" => "align_center"));
			$form_container->construct_row();
		}
	}
