...
if($mybb->input['action'] == 'enable')
{
	if(isset($mybb->input['id']) && is_numeric($mybb->input['id']))
	{
		...
		else
		{
			...
			if(!@is_dir(MYBB_ROOT.$result['f2gpath'].$result['imgfolder']) || !@is_writable(MYBB_ROOT.$result['f2gpath'].$result['imgfolder']))
			{
...
if($mybb->input['action'] == 'createtable')
{
	if(isset($mybb->input['id']) && is_numeric($mybb->input['id']))
	{
		...
		{
			//get custom fields
			$query = $db->simple_select("myshowcase_fields", "`name`, `field_type`, `max_length`, `require`", "setid=".$showcase['fieldsetid']);
...
if($mybb->input['action'] == 'deletetable')
{
	if(isset($mybb->input['id']) && is_numeric($mybb->input['id']))
	{
		...
		else
		{
			...
			$form = new Form("index.php?module=myshowcase-summary&amp;action=do_deletetable&amp;id=".$mybb->input['id'], "post", "do_deletetable");
	...
	if(isset($mybb->input['id']) && is_numeric($mybb->input['id']))
	{
		...
		else
		{
           ...
           $pattern = '!"#$%&\'( )*+,-./:;<=>?@[\]^_`{|}~';
            ...
            $showcase_name = preg_replace("/^[$pattern]+|[$pattern]+$/u",
                            "",
                            $showcase_name);

            // Replace middle punctuation with one separator.
            $showcase_name = preg_replace("/[$pattern]+/u",
                            '-',
                            $showcase_name);
...
//basic summary page, output regardless of action
{
	...
	$form_container->output_row_header($lang->myshowcase_summary_name, array("width" => "10%", "class" => "align_center"));
	$form_container->output_row_header($lang->myshowcase_summary_description, array("width" => "17%", "class" => "align_center"));
	$form_container->output_row_header($lang->myshowcase_summary_entries_count, array("width" => "5%", "class" => "align_center"));
	$form_container->output_row_header($lang->myshowcase_summary_comment_count, array("width" => "5%", "class" => "align_center"));
	$form_container->output_row_header($lang->myshowcase_summary_attachments_count, array("width" => "5%", "class" => "align_center"));
	$form_container->output_row_header($lang->myshowcase_summary_attachments_size, array("width" => "5%", "class" => "align_center"));
	$form_container->output_row_header($lang->myshowcase_summary_main_file, array("width" => "5%", "class" => "align_center"));
	$form_container->output_row_header($lang->myshowcase_summary_image_folder, array("width" => "5%", "class" => "align_center"));
	$form_container->output_row_header($lang->myshowcase_summary_forum_folder, array("width" => "5%", "class" => "align_center"));
	$form_container->output_row_header($lang->myshowcase_summary_field_set, array("width" => "5%", "class" => "align_center"));
	$form_container->output_row_header($lang->myshowcase_summary_status, array("width" => "2%", "class" => "align_center"));
	...
	else
	{
		while($result = $db->fetch_array($query))
		{
			...
			if($table_ready)
			{
				$query_myshowcase = $db->query("SELECT count(*) AS total FROM ".TABLE_PREFIX."myshowcase_data".$result['id']);
				$result2 = $db->fetch_array($query_myshowcase);
				$num_entries = ($result2 ? $result2['total'] : 0);

				$query_attach = $db->query("SELECT count(*) AS total, sum(filesize) AS totalsize FROM ".TABLE_PREFIX."myshowcase_attachments a, ".TABLE_PREFIX."myshowcase_data".$result['id']." d WHERE a.gid=d.gid AND a.id=".$result['id']." GROUP BY a.id");
				$result2 = $db->fetch_array($query_attach);
				$num_attach = ($result2 ? $result2['total'] : 0);
				$attach_size = ($result2 ? $result2['totalsize'] : 0);

				$query_comments = $db->query("SELECT count(*) AS total FROM ".TABLE_PREFIX."myshowcase_comments c, ".TABLE_PREFIX."myshowcase_data".$result['id']." d WHERE c.gid=d.gid AND c.id=".$result['id']." GROUP BY c.id");
			...
			$popup->add_item($lang->myshowcase_summary_edit, "index.php?module=myshowcase-edit&amp;action=edit-main&amp;id={$result['id']}");
			...
			if($result['enabled'] == 1)
			{
				...
				if($table_ready)
				{
					$popup->add_item($lang->myshowcase_summary_disable, "index.php?module=myshowcase-summary&amp;action=disable&amp;id={$result['id']}");
				}
			}
			else
			{
				...
				if($table_ready)
				{
					$popup->add_item($lang->myshowcase_summary_enable, "index.php?module=myshowcase-summary&amp;action=enable&amp;id={$result['id']}");
				}
			}
			...
			{
				$popup->add_item($lang->myshowcase_summary_deletetable, "index.php?module=myshowcase-summary&amp;action=deletetable&amp;id={$result['id']}");
			}

			$popup->add_item($lang->myshowcase_summary_seo, "index.php?module=myshowcase-summary&amp;action=show_seo&amp;id={$result['id']}");
			$popup->add_item($lang->myshowcase_summary_delete, "index.php?module=myshowcase-edit&amp;action=delete&amp;id={$result['id']}");
			...
			$form_container->output_cell(number_format($attach_size / 1024 / 1024, 2, '.', ','),array("class" => "align_center"));
			...
			$form_container->output_cell($result_fieldset['setname'].'<br />(ID='.$result['fieldsetid'].')',array("class" => "align_center"));
	...
	if(count($fieldsets) == 0)
	{
		...
		$form_container->output_row_header($lang->myshowcase_summary_name, array("width" => "25%", "class" => "align_center"));
		...
		$form_container->output_row_header($lang->myshowcase_summary_field_set, array("width" => "8%", "class" => "align_center"));

		$form_container->output_cell($lang->myshowcase_summary_nofieldsets, array("colspan" => "6", "class" => "align_center"));
		$form_container->construct_row();
		$form_container->end();
	...
	else
	{
		...
		$form_container->output_cell($form->generate_select_box('newfieldset', $fieldsets, "", array('id' => 'fieldsets')),array('class' => 'align_center'));
		$form_container->construct_row();
		$form_container->end();
