...
class MyShowcaseDataHandler extends DataHandler
{
	...
	* @var string
	*/
	var $language_file = 'datahandler_myshowcase';
	...
	* @var string
	*/
	var $language_prefix = 'myshowcasedata';
	 ...
	 * @var array
	 */
	var $myshowcase_insert_data = array();
	 ...
	 * @var array
	 */
	var $myshowcase_update_data = array();
			...
			if($field['enabled'] == 1 || $field['require'] == 1) //added require check in case admin sets require but not enable
			{
				...
				switch($field['field_type'])
				{
					...
					case 'varchar':
						...
						{
							if(my_strlen(strval($myshowcase_data[$fname])) > $field['max_length'] || my_strlen(strval($myshowcase_data[$fname])) < $field['min_length'] )
							{
								$this->set_error("invalid_length", array($field_header, my_strlen(strval($myshowcase_data[$fname])), $field['min_length'].'-'.$field['max_length']));
							}
						}
					break;
					...
					case 'text':
						...
						{
							$this->set_error("message_too_long", array($field_header, my_strlen($myshowcase_data[$fname]), $me->othermaxlength));
						}
						
						if(my_strlen(strval($myshowcase_data[$fname])) < $field['min_length'] )
						{
							$this->set_error("invalid_length_min", array($field_header, my_strlen(strval($myshowcase_data[$fname])), $field['min_length']));
						}
					break;
				}
			}
	...
	function update_showcase()
	{
		...
		if($myshowcase_data['posthash'])
		{
			...
			$db->update_query("myshowcase_attachments", $attachmentassign, "id=".$me->id." AND posthash='{$myshowcase_data['posthash']}'");
		}
