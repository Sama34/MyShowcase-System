...
//			exit;
	...
	case "reports":
	{
		...
		if($mybb->input['rid'])
		{
			$query = $db->simple_select("myshowcase_reports", "COUNT(rid) AS count", "rid <= '".$mybb->input['rid']."' AND id=".$me->id);
		...
		while($report = $db->fetch_array($query))
		{
			...
			$report['reporterlink']  = build_profile_link($report['reportername'], $report['reporteruid'], '','', $forumdir.'/');
			$report['authorlink'] = build_profile_link($report['authorname'], $report['authoruid'], '','', $forumdir.'/');
