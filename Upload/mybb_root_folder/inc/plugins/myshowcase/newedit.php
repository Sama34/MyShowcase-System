...
switch($mybb->input['action'])
{
	...
	case "new":
	{
		...
		{
			$showcase_editing_user = str_replace("{username}", $showcase_user['username'], $lang->myshowcase_editing_user);
		...
		if(($mybb->input['action'] == "new" && $me->userperms['canadd']) || ($mybb->input['action'] == "edit" &&($me->userperms['canedit'] || $me->userperms['canmodedit'])))
		{
			...
			if($can_add_attachments)
			{
				...
				while($attachment = $db->fetch_array($query))
				{
					...
					$attachment['icon'] = str_replace('<img src="', '<img src="'.$mybb->settings['bburl'].'/', $attachment['icon']);
					
					$attach_mod_options = '';
					if($attachment['visible'] != 1)
					{
						eval("\$attachments .= \"".$templates->get("myshowcase_new_attachments_attachment_unapproved")."\";");
					}
					else
					{
				...
				$lang->myshowcase_attach_quota = $lang->sprintf($lang->myshowcase_attach_quota, $attachcount, ($attach_limit == -1 ? $lang->myshowcase_unlimited : $attach_limit)).'<br>';
				if($attach_limit == -1 || ($attach_limit != -1 && $attachcount < $attach_limit))
				{
					if($me->userperms['canwatermark']  && $me->watermarkimage != "" && @file_exists($me->watermarkimage))
					{
						eval("\$showcase_watermark = \"".$templates->get("myshowcase_watermark")."\";");
					}
					eval("\$showcase_new_attachments_input = \"".$templates->get("myshowcase_new_attachments_input")."\";");
				}
			...
			foreach ($showcase_fields_enabled as $fname => $ftype)
			{
				...
				{
					$mybb->input['myshowcase_field_'.$fname] = htmlspecialchars_uni(stripslashes($showcase_data[$fname]));
				}
				...
				switch( $ftype)
				{
					...
					case "radio":
						...
						$query = $db->simple_select("myshowcase_field_data", "*", "setid=".$me->fieldsetid." AND name='".$fname."' AND valueid != 0", array("order_by" => "disporder"));
						if($db->num_rows($query) == 0)
						{
					...
					case "date":
						...
						for ($i = 1; $i <= 12; $i++)
						{
							$showcase_field_options_m .= '<option value="'.substr('0'.$i, -2).'" '.($showcase_field_value_m == substr('0'.$i, -2) ? ' selected' : '') .'>'.substr('0'.$i, -2).'</option>';
						}
						for ($i = 1; $i <= 31; $i++)
						{
							$showcase_field_options_d .= '<option value="'.substr('0'.$i, -2).'" '.($showcase_field_value_d == substr('0'.$i, -2) ? ' selected' : '') .'>'.substr('0'.$i, -2).'</option>';
						}
	...
	case "do_newshowcase":
	{
		...
		if($mybb->request_method == "post" && $mybb->input['submit'])
		{
			...
			else
			{
				...
				$redirect_newshowcase = $lang->redirect_myshowcase_new.''.$lang->redirect_myshowcase.''.$lang->sprintf($lang->redirect_myshowcase_return, $showcase_url);
