...
function myshowcase_remove_attachment($gid, $posthash, $aid)
{
	...
	if($posthash != "")
	{
		$query = $db->simple_select("myshowcase_attachments", "aid, attachname, thumbnail, visible", "id='{$me->id}' AND aid='{$aid}' AND posthash='{$posthash}'");
		$attachment = $db->fetch_array($query);
	}
	else
	{
		$query = $db->simple_select("myshowcase_attachments", "aid, attachname, thumbnail, visible", "id='{$me->id}' AND aid='{$aid}' AND gid='{$gid}'");
		$attachment = $db->fetch_array($query);
	}
...
function myshowcase_upload_attachment($attachment, $update_attachment=false, $dowatermark=false)
{
	...
	{
		$query = $db->simple_select("myshowcase_attachments", "SUM(filesize) AS ausage", "uid='".intval($showcase_uid)."'");
	...
	$query = $db->simple_select("myshowcase_attachments", "*", "filename='".$db->escape_string($attachment['name'])."' AND id='{$me->id}' AND (posthash='$posthash' OR (gid='".intval($gid)."' AND gid!='0'))");
	...
	if($ext == "gif" || $ext == "png" || $ext == "jpg" || $ext == "jpeg" || $ext == "jpe")
	{
			...
			{	
				...
				switch($img_type)
				{
 				...
 				if($image)
 				{
					...
					imagecopy($image, $watermark, $dest_x, $dest_y, 0, 0, min($wmwidth, $size[0]), min($wmheight, $size[1]));
		...
		$thumbnail = generate_thumbnail($me->imgfolder."/".$filename, $me->imgfolder, $thumbname, $me->thumb_height, $me->thumb_width);
	...
	if($prevattach['aid'] && $update_attachment == true)
	{
		...
		$db->update_query("myshowcase_attachments", $attacharray, "aid='".$db->escape_string($prevattach['aid'])."'");
		$aid = $prevattach['aid'];
	}
	else
	{
