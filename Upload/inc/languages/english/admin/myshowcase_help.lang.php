...
<li>Add .htaccess entries for SEO friendly URLs, though the MyShowcase System will provide content for the admin to copy/paste into .htaccess
</ul>
<h3>Miscellaneous</h3>
<p />
There are a large number of templates included with the MyShowcase System. These templates are installed when your Activate & Install the MyShowcase System plugin. These templates are installed as Master Templates into a new MyShowcase template group. This allows the templates to be applicable to any template set and customized independently within each set. During an upgrade of the MyShowcase System, these master templates may be updated and thus any modified templates may become out of date and some functionality lost. If this occurs, you should make note of your customizations, revert the template and reapply the customizations.
<p />
...
<br />
From the Summary sub-menu, enter a Name, Description, Script File, Image Folder, Relative Path and select a Field Set. Click Add a Showcase.
<ul>
...
<li><strong>Script File:</strong> is the name of the PHP file that will represent the Showcase and to which you will link your visitors to. The MyShowcase System comes with a file called showcase.php that is uploaded to the MyBB root folder. To use this file as-is for a single Showcase install, specify the Script File field as 'showcase.php' (without the quotes). To use multiple showcases, copy and rename this file, for example, dock.php for boating use or user_pc.php for computer details, and use that as the Script File. You do <strong>NOT</strong> include path information here.
<li><strong>Image Folder:</strong> is the path, relative to the specified Script File, that will be used for storing attachments. You must create this folder and set the appropriate write permissions so images can be uploaded there. You <strong>MUST</strong> use a separate Image Folder for each showcase you create in order to prevent overwriting or corrupting data.
<li><strong>Relative Path from Forum:</strong> is the path from the MyBB root folder to the Script File. This allows the showcase script file to be located outside the MyBB root folder and still be linked inside MyBB posts and portal. If you setup the Script File to be located outside the MyBB root folder, you must edit the Script File directly and specify the relative path to the forums. Any required edits are commented as such in the file.
</ul>
As each showcase created can have any table schema given the field set assigned, once you have created the showcase in the MyShowcase Admin and assigned a complete field set, you need to create the data table. In the Summary sub-menu, for the showcase you want to create, click the drop down menu in the Controls field and select Create Table. Once you create the table, which will be named <mybb_prefix>_myshowcase_dataXX where XX is the ID of the given showcase, you can then enable the showcase for use via the same Controls menu. This will not create the link to the showcase, only enable it for use. You need to insert a link to the showcase yourself via the MyBB template editor. This same menu will allow you to disable a specific showcase, such as for maintenance.
...
<br />
The parser setting is to make the field run through the MyBB parser so that, if enabled, smiles, BBCode and HTML are parsed and converted as required. This applies to text based fields only. Each showcase has settings for allowing smiles, BBCode and HTML. Edit a Showcase from the Summary menu and switch to the 'Other' Tab and check the items you wish to be supported in the particular showcase.<br />
<br />
...
<br />
The MyShowcase System will handle labels for the fields you create, at least for the default language of the MyBB installation. These labels are used in the table header of the list view as well as the field prompts when creating, editing and viewing a showcase entry. The MyShowcase System will create a new language file for each field set and populate the file with the labels you enter/save.  If your MyBB installation supports multiple languages, you can then copy these generated files and translate them as you would with any other language file. MyShowcase System generated language files are located in the <mybb_root>\inc\languages\<default_lang>\ folder and use the naming convention 'myshowcase_fsXX.lang.php' where XX is the ID of the field set.<br />
<br />
...
<li><strong>Permissions:</strong> This tab controls user group based permissions.
...
<li><strong>Script File:</strong> This is the PHP file to be used for the showcase. There should be no path information here, file_name only.
<li><strong>Folder to Store Attachments:</strong> This is the location on the server that will be used to store attachments for the showcase. This should be relative to the Script File and must be writable by the webserver.
...
<li><strong>Watermark Image:</strong> If you wish to allow user groups to watermark their attachments, specify the image here. A transparent PNG is highly recommended. This image will be resized to fit attachments that are smaller.
...
<li><strong>Path to Forums:</strong> This is the relative path from the Forum Index to the Script File. This is used when 'Link this showcase in user details in posts' option is enabled.
...
<li><strong>Text Type Fields:</strong> This is the maximum number of characters allowed in a TextArea input field. In order to support multiple line input (think of a post message body) a text area form is required, but you can limit the number of characters input in order to preserve formatting or simply minimize content.
...
<li><strong>Allow Smiles:</strong> If a field is set to use the Parser, are smiles allowed?
...
<li><strong>Link this showcase in user details in posts:</strong> Creates a link in the user details area of the post bit that displays that users entries in the showcase. This used the Showcase Name and the number of entries the user has as the link.
<li><strong>Try to display a random entry in this showcase that has attachments on the portal:</strong> Attempt to find a random showcase that has an image attachment and display it on the portal. This setting requires some manual template edits to the portal as well as possible code changes to MYBB_ROOT/inc/plugins/myshowcase.php in order to properly display the contents to your liking. For assistance with this, visit the <a href="http://www.communityplugins.com/" target="_blank">Community Plugins</a> site for help.
</ul>
<br />
<h3>Permissions Tab</h3><br />
The MyShowcase System supports group based permissions and honors the built-in MyBB usergroup assignments, including additional groups. The MyShowcase System permissions setup uses the MyBB 1.4 style check boxes and is group specific. Permissions include:
<ul>
...
The MyShowcase System assumes full moderator permissions for the default MyBB Moderator, Super Moderator and Administrator groups. It also supports additional user and group based moderators, just like MyBB 1.6 forum permissions and honors the built-in MyBB user group assignments, including additional groups. The MyShowcase System permissions setup uses the MyBB 1.4 style check boxes and is group and showcase specific. Permissions include:
<ul>
<li><strong>Can Approve:</strong> Can the moderator or moderator group members approve and unapproved showcases.
...
However, it is possible to assign any user group as a moderator to a specific showcase and then reduce the permissions for that group. For example, the Administrator group can be added as a moderator group and then have all the moderation permissions revoked. The MyShowcase System will then not allow Administrators to moderate that specific showcase. Additionally, a specific user assigned as a moderator can have any of the permissions applied or revoked and those permissions will be used regardless of the user group that user is in and the permissions assigned to the group, either as default or specifically in the additional moderators section. 
<br />
EOT;

$l['myshowcase_help_cache'] = <<<EOT
<h3>Rebuild Cache</h3><br />
The MyShowcase System piggybacks on the built-in MyBB cache system in order to improve performance and reduce server load by minimizing the queries that are required. While it is expected that most operations to the MyShowcase System via the ACP will result in the cache being updated, it is possible that the cache will become out of date, especially after manual database edits. In this case, simply click Rebuild Cache in the sub-menu of the MyShowcase Admin and all MyShowcase related caches will be updated. You can view the current cache contents via the standard ACP functionality.<br />
<br />
...
<br />
If the MyBB installation supports multiple languages, the non-default languages will need to have their own language files created manually. Simple copy/paste the 'myshowcase' files from the default language folder to the other language folders. Then edit those file and translate as needed. There are language files in the main language folder as well as the /admin folder.<br />
<br />
All language files are also editable via the built-in MyBB language modification functionality. However, the on-the-fly language edits within the MyShowcase System require the proper filesystem permissions on the language folders in order to create/modify them. This is the same permissions required for the built-in MyBB language editor to work.
</ul>
EOT;